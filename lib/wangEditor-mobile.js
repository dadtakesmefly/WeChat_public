					...
					reader.onload = function(e){
						...
						if (window.URL && window.URL.createObjectURL) {
							...
							$focusElem.after(
								...
								'	<img src="' + prevImgSrc + '" style="opacity:0.3; max-width:100%;"/>' +
								'</div>'
							);
						} else {
			            ...
			            xhr.onload = function () {
			            	...
			            	if (resultSrc.indexOf('error|') === 0) {
								...
								loadImg.onerror = function () {
									...
									loadImg = document.createElement('img');
									//字符串转对象
									loadImg.src = JSON.parse(resultSrc).data;
									//console.log(JSON.parse(resultSrc));
									//console.log(JSON.parse(resultSrc).data);
									$loadImg = $(loadImg);
									$loadImg.css('max-width', '100%');
									$prevImgContainer.after($loadImg);
									//移除预览
